...
__global__ void batch_convolution_kernel(unsigned char* red, unsigned char* green, unsigned char* blue, unsigned char* gray,
    ...
    // Calculate global output row and column indices
    int dst = ty * OUT_TILE_DIM + tx;
    int dest_temp = dst;
    int dx = dest_temp % IN_TILE_DIM;
    dest_temp = dest_temp / IN_TILE_DIM;
    int dy = dest_temp;

    int Y_ip = dy + (by * OUT_TILE_DIM) - mask_radius;
    int X_ip = dx + (bx * OUT_TILE_DIM) - mask_radius;
    int input_idx = (Y_ip * width) + X_ip;

    // Load input into shared memory (handle halos)
    if (Y_ip >= 0 && Y_ip < height && X_ip >= 0 && X_ip < width) {
        red_tile[dy][dx] = red[image_offset + input_idx];
        green_tile[dy][dx] = green[image_offset + input_idx];
        blue_tile[dy][dx] = blue[image_offset + input_idx];
    } else {
        red_tile[dy][dx] = 0.0f;
        green_tile[dy][dx] = 0.0f;
        blue_tile[dy][dx] = 0.0f;
    }

    // Load additional halo regions if necessary
    dst = OUT_TILE_DIM * OUT_TILE_DIM + ty * OUT_TILE_DIM + tx;
    dest_temp = dst;
    dx = dest_temp % IN_TILE_DIM;
    dest_temp = dest_temp / IN_TILE_DIM;
    dy = dest_temp;

    Y_ip = dy + (by * OUT_TILE_DIM) - mask_radius;
    X_ip = dx + (bx * OUT_TILE_DIM) - mask_radius;
    input_idx = (Y_ip * width) + X_ip;
    ...
    float sum = 0.0f;
    int y, x;
    for (y = 0; y < mask_dimension; y++) {
        for (x = 0; x < mask_dimension; x++) {
            if (channel == 0) {
                sum += mask_c[y * mask_dimension + x] * red_tile[threadIdx.y + y][threadIdx.x + x];
            } else if (channel == 1) {
                sum += mask_c[y * mask_dimension + x] * green_tile[threadIdx.y + y][threadIdx.x + x];
            } else if (channel == 2) {
                sum += mask_c[y * mask_dimension + x] * blue_tile[threadIdx.y + y][threadIdx.x + x];
            }
        }
    }
    ...
    __syncthreads(); 
    int Y_op = ty + (by * OUT_TILE_DIM);
    int X_op = tx + (bx * OUT_TILE_DIM);

    if (Y_op < height && X_op < width) {
        if (channel == 0) {
            ...
            // use the weighted sum equation of greyscale
...
void rgb2gray_gpu(float *mask, unsigned char* red, unsigned char* green, unsigned char* blue, unsigned char* gray, 
    ...
    printf("Copy Data to Device: %lld ms\n", duration);


    // find maximum width and height of batch
    ...
    // Call the kernel
    start = high_resolution_clock::now();
    dim3 numThreadsPerBlock(OUT_TILE_DIM, OUT_TILE_DIM, CHANNELS);
    ...
    stop = high_resolution_clock::now();
    duration = duration_cast<milliseconds>(stop - start).count();
    printf("Copy Data to Host: %lld ms\n", duration);
            ...
            if (image_data) {
                if (channels == 3 || channels == 4) { // Accept both RGB and RGBA images
                    ...
                    if (channels == 4) {
                        ...
                        for (int i = 0; i < w * h; ++i) {
                            rgb_data[i * 3] = image_data[i * 4];       // Red 
                            rgb_data[i * 3 + 1] = image_data[i * 4 + 1]; // Green 
                            rgb_data[i * 3 + 2] = image_data[i * 4 + 2]; // Blue 
                        }
...
int main(int argc, char* argv[]) {
    ...
    const char* prefix_id = "K12_"; // Prefix for the output filenames
